...
const artifactNames = {
  ...
  mcpBundle: "cowart-mcp.mjs",
  widget: "cowart-widget.html",
  manifest: "release-manifest.json",
};

const temporaryRoot = await mkdtemp(path.join(tmpdir(), "cowart-release-artifacts-"));
...
function generatedMcpAppsAlias(generatedModulePath) {
  return {
    name: "cowart-generated-mcp-apps",
    setup(build) {
      build.onResolve(
...
async function writeArtifacts(artifacts) {
  ...
  console.log(`Built ${artifacts.size} Cowart release artifacts in ${GENERATED_DIR}.`);
}
...
async function checkArtifacts(expectedArtifacts) {
  ...
  if (stale.length > 0) {
    throw new Error(
      `Cowart release artifacts are missing or stale: ${stale.join(", ")}. Run npm run build:artifacts.`,
    );
  }
  console.log("OK: Cowart release artifacts match the current source.");
}
